...
j1Map2::j1Map2() : j1Module(), map_loaded(false)
{
	name.create("map");
}
...
bool j1Map2::Awake(pugi::xml_node& config)
{
...
iPoint j1Map2::PosConverter(int x, int y) {
	iPoint ret;
...
SDL_Rect j1Map2::GetTileRect(TileSet* tileset, int id)
{
	int num = id;
...
// Load new map
bool j1Map2::Load(const char* file_name)
{
	bool ret = true;
...
bool j1Map2::LoadObjectGroup(pugi::xml_node& node, ObjectGroup* objectgroup)
{
	bool ret = true;
...
{
	bool ret = true;
	pugi::xml_node map = map_file.child("map2");
...
bool j1Map2::LoadTilesetDetails(pugi::xml_node& tileset_node, TileSet* set)
{
	bool ret = true;
...
bool j1Map2::LoadTilesetImage(pugi::xml_node& tileset_node, TileSet* set)
{
	bool ret = true;
...
bool j1Map2::LoadLayer(pugi::xml_node& node, MapLayer* layer)
{
	bool ret = true;
